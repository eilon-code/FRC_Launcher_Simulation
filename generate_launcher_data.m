...
num_sims = length(ratio_velocity_to_spin_list) * length(v_list) * length(a_list);
allRows = cell(num_sims, 1);
count = 1;

for ang = a_list
    for v = v_list
        for omega = ratio_velocity_to_spin_list .* v
    ...
    % Progress update
    fprintf('Finished Angle = %.1f (%d/%d)\n', ang, count-1, num_sims);
end
